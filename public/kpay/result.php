...
if ( $result == "CAPTURED" )
{
    $result_url = $config['base_url']."/payment/success";
   ...
   $clink = 'https://baniwail.com/conferm/?token='.$PaymentID.'&paymethod=Knet&saller='.$saller .'&status=Captured';
...
{
    $result_url = $config['base_url']."/payment/fail";
    $result_params = "?PaymentID=" . $PaymentID . "&Result=" . $result . "&PostDate=" . $postdate . "&TranID=" . $tranid . "&Auth=" . $auth . "&Ref=" . $ref . "&TrackID=" . $trackid . "&UDF1=" . $udf1 . "&UDF2=" .$udf2  . "&UDF3=" . $udf3  . "&UDF4=" . $udf4 . "&UDF5=" . $udf5  ;
     $clink = 'https://baniwail.com/cart';
    header('Location:'.$clink);
